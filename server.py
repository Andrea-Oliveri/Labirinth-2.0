...
def accept_client(link, connected_clients, level, players):
    """Function that accepts a connection asked by a client, adds its socket to
    the list connected_clients, sends him a confirmation message and creates a
    new player for him."""
...
def remove_client(client_link, connected_clients, players):
    """Function that forgets the client passed as parameter, erasing it from
    connected_clients and players and closing its connection."""
    players.pop(client_link)
    connected_clients.remove(client_link)
    client_link.close()
    
    
def send_clients_game_over(connected_clients, level, players):
    """Function that sends an end game message to each connected client,
    which varies depending on whether his player is on an exit or not.
    It also prints an informative message on the server terminal."""
...
def tell_clients(codon, connected_clients, level, players):
...
if __name__ == "__main__":
    ...
    try:
        connected_clients = []
        players = {}
        wait_for_players(connected_clients, level, players)
        run_game(connected_clients, level, players)
        main_link.close()
        os.system('pause')
        
    except KeyboardInterrupt:
        # In case of a keyboard interrupt, we want to close the socket.
        tell_clients('server quit', connected_clients, level, players)
        main_link.close()
